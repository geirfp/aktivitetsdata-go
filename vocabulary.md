...
deltok|Angir at deltakeren var til stede ved en virtuell eller fysisk hendelse eller aktivitet.
...
kommenterte|Angir at deltakeren leverte digitale eller skrevne anførsler på eller om et objekt.
...
fullførte|Angir at deltakeren fullførte eller avsluttet aktiviteten normalt.
...
avsluttet|Angir at deltakeren med vilje forlot en aktivitet eller et objekt.
...
erfarte|Angir at deltakeren kun påtraff objektet; kan anvendes i situasjoner der det ikke kreves spesielle resultater eller fullføring.
...
strøk|Angir at deltakeren ikke fullførte en aktivitet på et forutbestemt prestasjonsnivå.
...
innførte|Angir at deltakeren førte et objekt inn på et fysisk eller virtuelt sted.
...
oppstartet|Angir at leverandøren av aktiviteten har fastslått at deltakeren har startet en aktivitet tilfredsstillende.
...
samhandlet|Angir at deltakeren benyttet et fysisk eller virtuelt objekt.
...
begynte|Angir at deltakeren forsøkte å starte en aktivitet.
...
logget inn|Angir at deltakeren fikk tilgang til et system eller en tjeneste gjennom identifisering og autentisering med identifikator lagt fram av deltakeren.
...
logget ut|Angir at deltakeren enten mistet eller avsluttet tilgang til et system eller en tjeneste.
...
mestret|Angir høyeste nivå av forståelse eller kompetanse for deltakerens prestasjon i en aktivitet.
...
besto|Angir at deltakeren har lykkes i å gjennomføre en aktivitet på et forutbestemt nivå av tilfredsstillelse.
...
foretrakk|Angir valgte eller foretrukne muligheter eller innstillinger for en deltaker i forhold til et objekt eller en aktivitet.
...
nådde|Angir en verdi for hvor mye en deltaker har gått fram eller endret seg gjennom en aktivitet.
...
registrerte|Angir at deltakeren er offisielt innskrevet eller innført i en aktivitet.
...
besvarte|Angir at en deltaker reagerte eller svarte på et objekt.
...
delte|Angir deltakerens hensikt om å gi åpen tilgang til et objekt av felles interesse for andre deltakere eller grupper.
...
utsatte|Angir status for en aktivitet som er midlertidig stanset, når deltakeren har til hensikt å komme tilbake til objektet eller aktiviteten på et senere tidspunkt.
...
avsluttet|Angir at deltakeren har fullført en aktivitet tilfredsstillende.
...
avsto|Angir at kravene for læringsaktiviteten ble tilfredsstilt på annen måte enn ved å fullføre aktiviteten. Avstått blir brukt for å angi at deltakeren kan hoppe over aktiviteten.
...
vurdering|En vurdering er en aktivitetstype som fastsetter en elevs mestring av et visst fagområde. En vurdering inneholder typisk ett eller flere spørsmål.
...
forsøk|Et forsøk er særskilt sett av læringserfaringer i en aktivitet. Denne aktiviteten gir systemene mulighet til å identifisere erfaringer unikt når de kan ha oppstått i ulike interaksjoner med samme aktivitet.
...
interaction|An interaction is typically a part of a larger activity (such as a assessment, game, or simulation) and refers to a control to which a learner provides input. An interaction can be either an asset or function independently.|http://adlnet.gov/expapi/activities/interaction|https://w3id.org/xapi/adl 
...
leksjon|En lekse er læringsinnhold som eventuelt kan anta form av en SCO (formell, sporet læring). En lekse kan være frittstående eller være del av et større kurs.
...
lenke|En lenke er helt enkelt en måte å uttrykke kopling til en annen ressurs i eller utenfor en aktivitet. En lenke forutsetter ikke at en annen ressurs lanseres og bør anses som utenfor den aktuelle ressursen. Lenker er ikke læringsinnhold eller SCOer. Dersom en lenke er tiltenkt et slikt formål, bør den kategoriseres annerledes.
...
medium|Medium viser til tekst, lyd eller video som benyttes til å formidle informasjon. Et medium kan forbrukes (sporing: fullført), men har ikke en interaktiv komponent som kan resultere i skåre, vellykket eller stryk.
...
formål|Et formål fastslår om det er oppnådd kompetanse på et visst område. Formål blir typisk knyttet til spørsmål og vurderinger. Formål er ikke læringsinnhold og kan ikke være SCOer.
...
spørsmål|Et spørsmål er som regel del av en vurdering og krever et svar fra eleven; det blir så vurdert om svaret er korrekt.
